...
class YTVOS:
    ...
    def showAnns(self, anns, draw_bbox=False):
        ...
        if len(anns) == 0:
            return 0
        if 'segmentations' in anns[0] or 'keypoints' in anns[0]:
        ...
        if datasetType == 'instances':
            ...
            for ann in anns:
                ...
                if 'segmentations' in ann:
                    if type(ann['segmentations']) == list:
                        # polygon
                        for seg in ann['segmentations']:
                        ...
                        if type(ann['segmentations']['counts']) == list:
                            rle = maskUtils.frPyObjects([ann['segmentations']], ann['height'], ann['width'])
                        else:
                            rle = [ann['segmentations']]
                        m = maskUtils.decode(rle)
                ...
                if draw_bbox:
                    [bbox_x, bbox_y, bbox_w, bbox_h] = ann['bboxes']
